...
#  do
   #   tail -n+3 "$file"  >> /home/ubuntu/db_files/incoming/px/dev/CardActivity.csv
#  done
...
rm -f /home/ubuntu/db_files/incoming/px/dev/CardActivity*.csv
echo 'CARDACTIVITY -dev- DATA FILES DELETED, CREATING TEMP TABLE PRIMARY KEY'

#### ADD AN INDEXED PRIMARY KEY FIELD TO SPEED UP QUERIES, DELETE BEFORE INSERTING DATA INTO LIVE TABLE
mysql  --login-path=local --silent -DSRG_Dev -N -e "ALTER TABLE CardActivity_Temp ADD row_count INT(11) NOT NULL AUTO_INCREMENT AFTER SVDiscountTrackingBalance, ADD PRIMARY KEY (row_count)"
...
mysql  --login-path=local --silent -DSRG_Dev -N -e "ALTER TABLE CardActivity_Temp ADD COLUMN RowID int(11) AFTER POSkey"
echo 'RowID ADDED, DROPPING INDEXED PRIMARY KEY'

# REMOVE THE TEMP INDEX FIELD
mysql  --login-path=local --silent -DSRG_Dev -N -e "ALTER TABLE CardActivity_Temp DROP row_count"
...
mysql  --login-path=local --silent -DSRG_Dev -N -e "SELECT RIGHT(CheckNumber, 4), DOB, LocationID FROM CheckDetail_Live WHERE CheckDetail_Live.CheckNumber like '100%'" | while read -r CheckNumber DOB LocationID;
do
mysql  --login-path=local --silent -DSRG_Dev -N -e "UPDATE CardActivity_Live SET CheckNo=CONCAT('100',CheckNo) WHERE CheckNo = '$CheckNumber' AND TransactionDate = '$DOB' AND LocationID = '$LocationID' AND char_length(CheckNo) < '6'"
done
...
##### Update POSkey field (location + TransactionDate[excel format][no decimal] + checknumber)
mysql  --login-path=local --silent -DSRG_Dev -N -e "UPDATE CardActivity_Live set POSkey = CONCAT_WS('', LocationID, Exceldate, CheckNo)"
...
mysql  --login-path=local --silent -DSRG_Dev -N -e "INSERT INTO SRG_px.CardActivity_squashed
SELECT
