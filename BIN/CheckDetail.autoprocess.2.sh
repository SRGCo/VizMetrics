...
mysql  --login-path=local --silent -DSRG_checks -N -e "DROP TABLE IF EXISTS TableTurns_Temp"

### Create a empty copy of TableTurns_Temp table from TableTurns_Structure table
mysql  --login-path=local --silent -DSRG_checks -N -e "CREATE TABLE TableTurns_Temp AS (SELECT * FROM TableTurns_Structure WHERE 1=0)"

### Load the data from the latest file into the (temp) TableTurns table ########################
mysql  --login-path=local --silent -DSRG_checks -N -e "Load data local infile '/home/ubuntu/db_files/incoming/TableTurns.csv' into table TableTurns_Temp fields terminated by ',' lines terminated by '\n'"

### PUT DOB INTO SQL FORMAT
mysql  --login-path=local --silent -DSRG_checks -N -e "UPDATE TableTurns_Temp SET DOB= STR_TO_DATE(DOB, '%c/%e/%Y') WHERE STR_TO_DATE(DOB, '%c/%e/%Y') IS NOT NULL"

### Change DOB field to type date
mysql  --login-path=local --silent -DSRG_checks -N -e "ALTER TABLE TableTurns_Temp CHANGE DOB DOB DATE"

#### Change OpenTime & CloseTime to SQL format
mysql  --login-path=local --silent -DSRG_checks -N -e "UPDATE TableTurns_Temp SET CloseTime= STR_TO_DATE(CloseTime, '%m/%e/%Y %l:%i:%s %p') WHERE STR_TO_DATE(CloseTime, '%m/%e/%Y %l:%i:%s %p')"
mysql  --login-path=local --silent -DSRG_checks -N -e "UPDATE TableTurns_Temp SET OpenTime= STR_TO_DATE(OpenTime, '%m/%e/%Y %l:%i:%s %p') WHERE STR_TO_DATE(OpenTime,  '%m/%e/%Y %l:%i:%s %p')"

##### MUST ALTER THESE FIELDS TO DATETIME #################
mysql  --login-path=local --silent -DSRG_checks -N -e "ALTER TABLE TableTurns_Temp CHANGE CloseTime CloseTime DATETIME NOT NULL"

#### Create POSkey field          ######################### INDEX #######################
mysql  --login-path=local --silent -DSRG_checks -N -e "ALTER TABLE TableTurns_Temp ADD POSkey VARCHAR(30) first"
mysql  --login-path=local --silent -DSRG_checks -N -e "ALTER TABLE TableTurns_Temp ADD INDEX(POSkey)"

#### Create excel date field
mysql  --login-path=local --silent -DSRG_checks -N -e "ALTER TABLE TableTurns_Temp ADD Exceldate INT(100) NOT NULL AFTER LocationID"

#### Update excel date field
mysql  --login-path=local --silent -DSRG_checks -N -e "UPDATE TableTurns_Temp set Exceldate = (((unix_timestamp(DOB) / 86400) + 25569) + (-5/24))"
...
mysql  --login-path=local --silent -DSRG_checks -N -e "UPDATE TableTurns_Temp set POSkey = CONCAT_WS('', LocationID, Exceldate, CheckNumbers)"
mysql  --login-path=local --silent -DSRG_checks -N -e "ALTER TABLE TableTurns_Temp ADD INDEX(POSkey)"
...
mysql  --login-path=local --silent -DSRG_checks -N -e "CREATE TABLE CheckDetail_Temp AS (SELECT * FROM CheckDetail_Structure WHERE 1=0)"
...
mysql  --login-path=local --silent -DSRG_checks -N -e "DELETE from CheckDetail_Temp where CheckNumber = '0'"

#### Change TransactionDate field to type date
mysql  --login-path=local --silent -DSRG_checks -N -e "ALTER TABLE CheckDetail_Temp CHANGE DOB DOB DATE"

#### Create EMPLOYEE fields
mysql  --login-path=local --silent -DSRG_checks -N -e "ALTER TABLE CheckDetail_Temp ADD PayrollID VARCHAR( 26 ) first"
mysql  --login-path=local --silent -DSRG_checks -N -e "ALTER TABLE CheckDetail_Temp ADD firstname VARCHAR( 255 ) first"
mysql  --login-path=local --silent -DSRG_checks -N -e "ALTER TABLE CheckDetail_Temp ADD lastname VARCHAR( 255 ) first"

#### Create POSkey field         ######################### INDEX #######################
mysql  --login-path=local --silent -DSRG_checks -N -e "ALTER TABLE CheckDetail_Temp ADD POSkey VARCHAR(30) first"
mysql  --login-path=local --silent -DSRG_checks -N -e "ALTER TABLE CheckDetail_Temp ADD INDEX(POSkey)"

#### Create excel date fieldC
mysql  --login-path=local --silent -DSRG_checks -N -e "ALTER TABLE CheckDetail_Temp ADD Exceldate INT(100) NOT NULL AFTER LocationID"

#### Update excel date field
mysql  --login-path=local --silent -DSRG_checks -N -e "UPDATE CheckDetail_Temp set Exceldate = (((unix_timestamp(DOB) / 86400) + 25569) + (-5/24))"

#### Update POSkey field (location + TransactionDate[excel format][no decimal] + checknumber)
mysql  --login-path=local --silent -DSRG_checks -N -e "UPDATE CheckDetail_Temp SET POSkey = CONCAT_WS('', LocationID, Exceldate, CheckNumber)"

#### DROP EXCELDATE FIELD
mysql  --login-path=local --silent -DSRG_checks -N -e "ALTER TABLE CheckDetail_Temp DROP COLUMN Exceldate"

#### NAMES QUERIES/UPDATES 
mysql  --login-path=local --silent -DSRG_checks -N -e "UPDATE CheckDetail_Temp CDT 
...
mysql  --login-path=local --silent -DSRG_checks -N -e "INSERT INTO CheckDetail_Live SELECT * FROM CheckDetail_Temp"

#### UPDATE LIVE CHECK DETAIL WITH LIVE TABLE TURNS
mysql  --login-path=local --silent -DSRG_checks -N -e "UPDATE CheckDetail_Live CDL
...
mysql  --login-path=local --silent -DSRG_checks -N -e "UPDATE CheckDetail_Live CDL SET CDL.OpenTime = CDL.DOB WHERE CDL.OpenTime < '2001-01-01'"
