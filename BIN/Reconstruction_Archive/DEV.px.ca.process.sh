...
#### THIS IS A DEV SCRIPT IT WILL SQUASH ALL THE WAY BACK TO INCEPTION ####
	...
	echo 'DEV SQUASH UPDATED FROM INCEPTION'


	######## FIX CHECKS THAT WERE OPEN ACROSS MIDNIGHT
	mysql  --login-path=local -DSRG_Prod_Dev -N -e "SELECT DISTINCT(CardNumber) FROM CardActivity_squashed WHERE CardNumber IS NOT NULL AND TransactionTime > '21:00:00'													ORDER BY CardNumber ASC" | while read -r CardNumber;
	do
...
#### ALL THE WAY BACK TO INCEPTION FOR DEV 2ND SQUASH ALSO
