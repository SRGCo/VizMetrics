...
for file in /home/ubuntu/db_files/incoming/first_use/CardActivity*.csv
  do
	#### MAKE A COPY OF THE FILE IN BACKUP DIR
	cp "$file" //home/ubuntu/db_files/incoming/first_use/backup/
	tail -n+2 "$file"  >> /home/ubuntu/db_files/incoming/first_use/Infile.CardActivity.csv
...
rm -f /home/ubuntu/db_files/incoming/first_use/CardActivity*.csv
trap 'failfunction ${?} ${LINENO} "$BASH_COMMAND"' ERR

rm -f /home/ubuntu/db_files/incoming/first_use/Infile.CardActivity.csv
trap 'failfunction ${?} ${LINENO} "$BASH_COMMAND"' ERR
