...
set -e
######## WE NEED TO COPY CARDACTIVITY FILE (INTO DEV) SINCE LAST DATE IN DEV
###### WE RUN IT ENTIRELY IN DEV
...
mysql  --login-path=local --silent -DSRG_Dev -N -e "DELETE FROM CardActivity_Temp WHERE (CheckNo = 'web' or CheckNo = 'iOS' or CheckNo = 'Android')"
trap 'failfunction ${?} ${LINENO} "$BASH_COMMAND"' ERR
