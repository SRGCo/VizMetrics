...
#### WE CAN PROCESS GUEST INFO AS ITS OWN SUBROUTINE TO AVOID FAILURES STOPPING WHOLE SCRIPT
	...
	mysql  --login-path=local --silent -DSRG_Prod_Dev -N -e "DROP TABLE IF EXISTS Guests_temp"
	...
	mysql  --login-path=local --silent -DSRG_Prod_Dev -N -e "CREATE TABLE Guests_temp LIKE Guests_Structure"
	...
	mysql  --login-path=local --silent -DSRG_Prod_Dev -N -e "Load data local infile '/home/ubuntu/db_files/incoming/px/guests.infile.csv' into table Guests_temp fields terminated by ','  lines terminated by '\n'"
	echo 'Guests_temp loaded'


	### UPDATE TO NULLS FOR ZERO DATES FOR ANNIVERSARY
	mysql  --login-path=local --silent -DSRG_Prod_Dev -N -e "UPDATE Guests_temp SET AnniversaryDate = NULL WHERE CAST(AnniversaryDate AS CHAR(10)) = '0000-00-00'"
	trap 'failfunction ${?} ${LINENO} "$BASH_COMMAND"' ERR

	## UPDATE TO NULLS FOR ZERO DATES FOR REG DATE
	mysql  --login-path=local --silent -DSRG_Prod_Dev -N -e "UPDATE Guests_temp SET RegisterDate = NULL WHERE CAST(RegisterDate AS CHAR(10)) = '0000-00-00'"
	trap 'failfunction ${?} ${LINENO} "$BASH_COMMAND"' ERR

	### UPDATE TO NULLS FOR ZERO DATES FOR ENROLL DATE
	mysql  --login-path=local --silent -DSRG_Prod_Dev -N -e "UPDATE Guests_temp SET EnrollDate = NULL WHERE CAST(EnrollDate AS CHAR(10)) = '0000-00-00'"
	trap 'failfunction ${?} ${LINENO} "$BASH_COMMAND"' ERR

	### UPDATE TO NULLS FOR ZERO DATES FOR BIRTHDATE
	mysql  --login-path=local --silent -DSRG_Prod_Dev -N -e "UPDATE Guests_temp SET DateofBirth = NULL WHERE CAST(DateofBirth AS CHAR(10)) = '0000-00-00'"
	trap 'failfunction ${?} ${LINENO} "$BASH_COMMAND"' ERR
	...
	mysql  --login-path=local -DSRG_Prod_Dev -N -e "SELECT DISTINCT(CardNumber) FROM Guests_temp" | while read -r CardNumber;
	do
		# DELETE CARDS FROM GUEST MASTER IF ALREADY EXISTS
		mysql  --login-path=local --silent -DSRG_Prod_Dev -N -e "DELETE from Guests_Master WHERE CardNumber = '$CardNumber'"
		# INSERT LATEST INFO ABOUT THIS GUEST
		mysql  --login-path=local -DSRG_Prod_Dev -N -e "INSERT INTO Guests_Master SELECT Guests_temp.*,NULL,NULL,NULL FROM Guests_temp WHERE CardNumber = '$CardNumber'"
		#### UPDATE TOWN INFO IN GUESTS_MASTER FOR THIS CARD
		mysql  --login-path=local -DSRG_Prod_Dev -N -e "SELECT Zip FROM Guests_Master WHERE CardNumber = '$CardNumber'" | while read -r Zip;
		do
			mysql  --login-path=local -DSRG_Prod_Dev -N -e "SELECT Population, AvgIncome, Town FROM MA_Zips WHERE Zip = '$Zip'" | while read -r population income town;
			do
				mysql  --login-path=local -DSRG_Prod_Dev -N -e "UPDATE Guests_Master SET Population = '$population', AvgIncome = '$income', Town = '$town' WHERE CardNumber = '$CardNumber'"
			done	
		done 
...
# Delete Temp table if it exists
mysql  --login-path=local --silent -DSRG_Prod_Dev -N -e "DROP TABLE IF EXISTS Master_temp"
trap 'failfunction ${?} ${LINENO} "$BASH_COMMAND"' ERR
echo 'MASTER TEMP TABLE DROPPED'

# Create a empty copy of CardActivity table from CardActivityStructure table
mysql  --login-path=local --silent -DSRG_Prod_Dev -N -e "CREATE TABLE Master_temp LIKE Master_structure"
trap 'failfunction ${?} ${LINENO} "$BASH_COMMAND"' ERR
echo 'MASTER TEMP CREATED'

###### WE ONLY GET THE LAST WEEKS WORTH OF DATA
mysql  --login-path=local -DSRG_Prod_Dev -N -e "INSERT INTO Master_temp SELECT CD.*, CA.* FROM CheckDetail_Live AS CD 
...
mysql  --login-path=local --silent -DSRG_Prod_Dev -N -e "ALTER TABLE Master_temp ADD EnrollDate VARCHAR(11)"
trap 'failfunction ${?} ${LINENO} "$BASH_COMMAND"' ERR

# Create ACCOUNT STATUS
mysql  --login-path=local --silent -DSRG_Prod_Dev -N -e "ALTER TABLE Master_temp ADD Account_status VARCHAR(15)"
trap 'failfunction ${?} ${LINENO} "$BASH_COMMAND"' ERR

# Create ACCOUNT STATUS INDEX
mysql  --login-path=local --silent -DSRG_Prod_Dev -N -e "ALTER TABLE Master_temp ADD INDEX(Account_status)"
trap 'failfunction ${?} ${LINENO} "$BASH_COMMAND"' ERR
echo 'MASTER TEMP ENROLLDATE AND ACCOUNT STATUS FIELDS CREATED'

# Create ACCOUNT STATUS
mysql  --login-path=local --silent -DSRG_Prod_Dev -N -e "ALTER TABLE Master_temp ADD Card_status VARCHAR(15)"
trap 'failfunction ${?} ${LINENO} "$BASH_COMMAND"' ERR


# AVOID DUPES DELETE SAME INTERVAL BACK
mysql  --login-path=local --silent -DSRG_Prod_Dev -N -e "DELETE FROM Master WHERE DOB >= DATE_SUB(CURDATE(), INTERVAL 90 DAY) "
trap 'failfunction ${?} ${LINENO} "$BASH_COMMAND"' ERR
echo 'MASTER TRUNCATED - USING DOB'

# AVOID DUPES DELETE SAME INTERVAL BACK
mysql  --login-path=local --silent -DSRG_Prod_Dev -N -e "DELETE FROM Master WHERE TransactionDate >= DATE_SUB(CURDATE(), INTERVAL 90 DAY) "
trap 'failfunction ${?} ${LINENO} "$BASH_COMMAND"' ERR
...
mysql  --login-path=local --silent -DSRG_Prod_Dev -N -e "INSERT INTO Master SELECT * FROM Master_temp"
trap 'failfunction ${?} ${LINENO} "$BASH_COMMAND"' ERR
...
mysql  --login-path=local -DSRG_Prod_Dev -N -e "UPDATE Master JOIN Guests_Master ON Master.CardNumber = Guests_Master.CardNumber 
...
## TRUNCATE GUESTS TABLE BEFORE LOADING W NEW
# Delete Temp table if it exists
mysql  --login-path=local --silent -DSRG_Prod_Dev -N -e "DROP TABLE IF EXISTS Px_exchanges_temp"
echo 'PX EXCHANGES TEMP TABLE DROPPED, STARTING NEW PX EXCHANGES TEMP TABLE CREATION'

# Create a empty copy of CardActivity table from CardActivityStructure table
mysql  --login-path=local --silent -DSRG_Prod_Dev -N -e "CREATE TABLE Px_exchanges_temp LIKE Px_exchanges_structure"
echo 'PX EXCHANGES TEMP TABLE CREATED, LOADING DATA FILE TO PX EXCHANGES TEMP TABLE'

# Load the data from the latest file into the (temp) CardActivity table
mysql  --login-path=local --silent -DSRG_Prod_Dev -N -e "Load data local infile '/home/ubuntu/db_files/incoming/px/Infile.MediaExchanges.csv' into table Px_exchanges_temp fields terminated by ','  lines terminated by '\n'"
echo 'PX EXCHANGES TEMP loaded'
	
#Load the temp data into the live table
mysql  --login-path=local -DSRG_Prod_Dev -N -e "INSERT INTO Px_exchanges SELECT * FROM Px_exchanges_temp"
...
mysql  --login-path=local -DSRG_Prod_Dev -N -e "UPDATE Master JOIN Excludes ON Master.CardNumber = Excludes.CardNumber SET Master.Account_status = 'Exclude' "
trap 'failfunction ${?} ${LINENO} "$BASH_COMMAND"' ERR
...
mysql  --login-path=local -DSRG_Prod_Dev -N -e "UPDATE Master SET CheckNumber = CheckNo_px WHERE CheckNumber IS NULL "
trap 'failfunction ${?} ${LINENO} "$BASH_COMMAND"' ERR
echo 'MASTER EMPTY CHECKNO POPULATED FROM PX DATA'


mysql  --login-path=local -DSRG_Prod_Dev -N -e "UPDATE Master SET LocationID = LocationID_px WHERE LocationID IS NULL "
trap 'failfunction ${?} ${LINENO} "$BASH_COMMAND"' ERR
echo 'MASTER EMPTY LOCATION ID POPULATED FROM PX DATA'

mysql  --login-path=local -DSRG_Prod_Dev -N -e "UPDATE Master SET POSkey = POSKey_px WHERE POSkey IS NULL "
trap 'failfunction ${?} ${LINENO} "$BASH_COMMAND"' ERR
echo 'MASTER EMPTY POS KEYS POPULATED FROM PX DATA'

mysql  --login-path=local -DSRG_Prod_Dev -N -e "UPDATE Master SET DOB = TransactionDate WHERE DOB IS NULL "
trap 'failfunction ${?} ${LINENO} "$BASH_COMMAND"' ERR
echo 'MASTER EMPTY DOB POPULATED FROM PX DATA'



mysql  --login-path=local -DSRG_Prod_Dev -N -e "UPDATE Master SET GrossSalesCoDefined = DollarsSpentAccrued WHERE GrossSalesCoDefined IS NULL 
...
mysql  --login-path=local -DSRG_Prod_Dev -N -e "SELECT Master.DOB FROM Master WHERE Master.DOB IS NOT NULL AND DOB >= DATE_SUB(NOW(),INTERVAL 45 DAY) 
				GROUP BY Master.DOB ORDER BY Master.DOB DESC" | while read -r DOB;
do

		######## GET FY FOR THIS DOB (DOB)
		FY=$(mysql  --login-path=local -DSRG_Prod_Dev -N -e "SELECT FY from Lunas WHERE DOB = '$DOB'")

		######## GET FY FOR THIS DOB (DOB)
		YLuna=$(mysql  --login-path=local -DSRG_Prod_Dev -N -e "SELECT YLuna from Lunas WHERE DOB = '$DOB'")

		######## GET FY FOR THIS DOB (DOB)
		Luna=$(mysql  --login-path=local -DSRG_Prod_Dev -N -e "SELECT Luna from Lunas WHERE DOB = '$DOB'")
		...
		mysql  --login-path=local -DSRG_Prod_Dev -N -e "UPDATE Master SET FY = '$FY',YLuna = '$YLuna', Luna='$Luna' WHERE Master.DOB = '$DOB'"
