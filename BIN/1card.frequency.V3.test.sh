...
mysql  --login-path=local -DSRG_Dev -N -e "SELECT CardNumber FROM Master_test2 WHERE Cardnumber = '6000227903295851'" | while read -r CardNumber;
do
	echo $CardNumber
	...
	MaxDate=$(mysql  --login-path=local -DSRG_Dev -N -e "SELECT MAX(TransactionDate) from Master_test2 WHERE CardNumber = '$CardNumber'")
		...
		SecondMax=$(mysql  --login-path=local -DSRG_Dev -N -e "SELECT DISTINCT(TransactionDate) from Master_test2 WHERE CardNumber = '$CardNumber' AND (Vm_VisitsAccrued = '1.0000' OR Vm_VisitsAccrued = '1') ORDER BY TransactionDate DESC limit 1,1") 
		...
		##### IF SECONDMAX HAS A VALUE
			...
			then
				...
				mysql  --login-path=local -DSRG_Dev -N -e "UPDATE Master_test2 SET FreqCurrent = DATEDIFF(NOW(), '$MaxDate'), FreqRecent = DATEDIFF('$MaxDate', '$SecondMax'), Freq12mos = '$PrevYear', FreqLifetime = '$Lifetime'  WHERE CardNumber = '$CardNumber'"			   echo first and second MAX $MaxDate, 2ND $SecondMax 3RD $thirdMax  Prevyr $PrevYear PrevLife $Lifetime
