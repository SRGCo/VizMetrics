...
# rm -f /home/ubuntu/db_files/SRG_Prod_bu.sql
mysqldump  --login-path=local -uroot SRG_Prod > /home/ubuntu/db_files/SRG_Prod_bu.sql
...
mysql  --login-path=local --silent -DSRG_Prod -N -e "SELECT RIGHT(CheckNumber, 4), DOB, LocationID FROM CheckDetail_Live WHERE CheckDetail_Live.CheckNumber like '100%' ORDER BY DOB ASC" | while read -r CheckNumber DOB LocationID;
do
