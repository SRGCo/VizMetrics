...
mysql  --login-path=local -DSRG_dev2 -N -e "SELECT Master.DOB FROM Master  WHERE Master.DOB IS NOT NULL GROUP BY Master.DOB ORDER BY Master.DOB DESC" | while read -r TransactionDate;
do
...
mysql  --login-path=local -DSRG_dev2 -N -e "SELECT DISTINCT(CardNumber) FROM Master WHERE CardNumber IS NOT NULL ORDER BY CardNumber ASC" | while read -r CardNumber;
do
