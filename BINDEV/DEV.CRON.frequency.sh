...
mysql  --login-path=local -DSRG_Dev -N -e "SELECT CardNumber FROM Master GROUP BY CardNumber HAVING COUNT(*) > 1 ORDER BY CardNumber ASC" | while read -r CardNumber;
do
	echo $CardNumber
	######## COUNT ACCRUED VISITS OVER PREVIOUS 12 MONTHS
	PrevYear=$(mysql  --login-path=local -DSRG_Dev -N -e "SELECT COUNT(TransactionDate) FROM Master WHERE CardNumber = '$CardNumber' AND TransactionDate >= DATE_SUB(NOW(),INTERVAL 1 YEAR) AND Vm_VisitsAccrued > '0'")

	######## COUNT VISITS OVER LIFETIME

	Lifetime=$(mysql  --login-path=local -DSRG_Dev -N -e "SELECT COUNT(TransactionDate) FROM Master WHERE CardNumber = '$CardNumber' AND Vm_VisitsAccrued > '0' ")
	...
	MaxDate=$(mysql  --login-path=local -DSRG_Dev -N -e "SELECT MAX(TransactionDate) from Master WHERE CardNumber = '$CardNumber'")
		...
		SecondMax=$(mysql  --login-path=local -DSRG_Dev -N -e "SELECT DISTINCT(TransactionDate) from Master WHERE CardNumber = '$CardNumber' AND Vm_VisitsAccrued = '1.0000' ORDER BY TransactionDate DESC limit 1,1") 
		...
		then
			...
			mysql  --login-path=local -DSRG_Dev -N -e "UPDATE Master SET FreqCurrent = DATEDIFF(NOW(), '$MaxDate'), Freq12mos = '$PrevYear', FreqLifetime = '$Lifetime'  WHERE CardNumber = '$CardNumber'"
			# echo $MaxDate $PrevYear, $Lifetime, Current updated $CardNumber
		
		##### IF SECONDMAX HAS A VALUE
		else
			##### GET 3RD TO MAX TRANSACTIONDATE
			ThirdMax=$(mysql  --login-path=local -DSRG_Dev -N -e "SELECT DISTINCT(transactiondate) from Master WHERE CardNumber = '$CardNumber' AND Vm_VisitsAccrued = '1.0000' ORDER BY TransactionDate DESC limit 2,1") 
			##### IF THIRDMAX IS NULL / EMPTY
			if [ -z "$ThirdMax" ]
			then
				##### UPDATE ONLY FIRST AND SECOND FREQUENCIES
				mysql  --login-path=local -DSRG_Dev -N -e "UPDATE Master SET FreqCurrent = DATEDIFF(NOW(), '$MaxDate'), FreqRecent = DATEDIFF('$MaxDate', '$SecondMax'), Freq12mos = '$PrevYear', FreqLifetime = '$Lifetime'  WHERE CardNumber = '$CardNumber'"
				# echo $MaxDate, $SecondMax $PrevYear, $Lifetime, Current, Recent updated $CardNumber

			##### IF THIRDMAX HAS A VALUE
			else
				##### UPDATE ALL FREQUENCIES
				mysql  --login-path=local -DSRG_Dev -N -e "UPDATE Master SET FreqCurrent = DATEDIFF(NOW(), '$MaxDate'), FreqRecent = DATEDIFF('$MaxDate', '$SecondMax'), FreqPrevious = DATEDIFF('$SecondMax', '$ThirdMax'), Freq12mos = '$PrevYear', FreqLifetime = '$Lifetime'   WHERE CardNumber = '$CardNumber'"
	...
	###### -N is the No Headers in Output option
	###### -e is the 'read statement and quit'
#	mysql  --login-path=local -DSRG_Dev -N -e "SELECT record_id, transactiondate FROM Master ORDER BY record_id ASC" | while read -r recordid, transactiondate ;
#	do
#		##### UPDATE HISTORICAL CURRENT FREQUENCIES
#		mysql  --login-path=local -DSRG_Dev -N -e "UPDATE Master SET Hist_current_freq = ('$transactiondate' - '$MaxDate') WHERE recordid = '$recordid'"
