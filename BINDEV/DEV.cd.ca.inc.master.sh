...
# Create CARD STATUS
mysql  --login-path=local --silent -DSRG_Dev -N -e "ALTER TABLE Master_temp ADD Card_status VARCHAR(26)"
trap 'failfunction ${?} ${LINENO} "$BASH_COMMAND"' ERR
...
mysql  --login-path=local --silent -DSRG_Dev -N -e "DELETE FROM Master WHERE DOB >= DATE_SUB(CURDATE(), INTERVAL 30 DAY) "
trap 'failfunction ${?} ${LINENO} "$BASH_COMMAND"' ERR
...
trap 'failfunction ${?} ${LINENO} "$BASH_COMMAND"' ERR
echo 'MASTER- EXCHANGED CARDS PROCESS/FIXED, CARD STATUS UPDATED TO -Exchange-'


######### EXCLUDES SECTION USE OR NOT ? ? ?
mysql  --login-path=local -DSRG_Dev -N -e "UPDATE Master JOIN Excludes ON Master.CardNumber = Excludes.CardNumber SET Master.Card_status = 'Exclude' "
# trap 'failfunction ${?} ${LINENO} "$BASH_COMMAND"' ERR
echo 'MASTER - NO ACCOUNTS EXCLUDED !!!!!!!!!!!!!!!'


######## UPDATE THE EMPTY CHECKDETAIL FIELDS WITH PX DATA
mysql  --login-path=local -DSRG_Dev -N -e "UPDATE Master SET CheckNumber = CheckNo_px WHERE CheckNumber IS NULL AND CheckNo_px IS NOT NULL "
trap 'failfunction ${?} ${LINENO} "$BASH_COMMAND"' ERR
echo 'MASTER EMPTY CHECKNO POPULATED FROM PX DATA'


mysql  --login-path=local -DSRG_Dev -N -e "UPDATE Master SET LocationID = LocationID_px WHERE LocationID IS NULL AND LocationID_px IS NOT NULL "
trap 'failfunction ${?} ${LINENO} "$BASH_COMMAND"' ERR
echo 'MASTER EMPTY LOCATION ID POPULATED FROM PX DATA'

mysql  --login-path=local -DSRG_Dev -N -e "UPDATE Master SET POSkey = POSKey_px WHERE POSkey IS NULL AND POSKey_px IS NOT NULL "
trap 'failfunction ${?} ${LINENO} "$BASH_COMMAND"' ERR
echo MASTER EMPTY POS KEYS POPULATED FROM PX DATA

mysql  --login-path=local -DSRG_Dev -N -e "UPDATE Master SET DOB = TransactionDate WHERE DOB IS NULL AND TransactionDate IS NOT NULL "
trap 'failfunction ${?} ${LINENO} "$BASH_COMMAND"' ERR
...
mysql  --login-path=local -DSRG_Dev -N -e "SELECT Master.DOB FROM Master WHERE Master.DOB IS NOT NULL AND DOB >= DATE_SUB(NOW(),INTERVAL 30 DAY) 
