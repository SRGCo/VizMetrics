...
mysql  --login-path=local -uroot  -DSRG_Dev -N -e "SELECT MIN(Guests), MAX(Guests), AVG(Guests), TableName, LocationID INTO OUTFILE '/home/ubuntu/db_files/outfiles/joined.cd.ca.csv' FIELDS TERMINATED BY ',' LINES TERMINATED BY '\n' FROM Master WHERE (Master.Account_status <> 'TERMIN'	AND Master.Account_status <> 'SUSPEN' 	AND Master.Account_status <> 'Exchanged' AND Master.Account_status <> 'Exchange' 	AND Master.Account_status <> 'Exclude') OR (Account_status IS NULL)	AND Master.Guests > '0' GROUP by Master.LocationID, Master.TableName"
