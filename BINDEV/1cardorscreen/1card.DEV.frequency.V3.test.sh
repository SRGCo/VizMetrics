...
mysql  --login-path=local -DSRG_Dev -N -e "SELECT CardNumber FROM Master_test WHERE CardNumber IS NOT NULL AND CardNumber = '6000227901477006' GROUP BY CardNumber ORDER BY CardNumber ASC" | while read -r CardNumber;
do
	echo $CardNumber
