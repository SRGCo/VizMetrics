...
do
	...
	MaxDate=$(mysql  --login-path=local -DSRG_Dev -N -e "SELECT MAX(TransactionDate) from Master_test WHERE CardNumber = '$CardNumber'")
		...
		SecondMax=$(mysql  --login-path=local -DSRG_Dev -N -e "SELECT TransactionDate from Master_test WHERE CardNumber = '$CardNumber' AND Vm_VisitsAccrued = '1' ORDER BY TransactionDate DESC limit 1,1") 
