...
mysql  --login-path=local -DSRG_Dev -N -e "SELECT Master.DOB FROM Master WHERE Master.DOB > '2013-09-01' GROUP BY Master.DOB ORDER BY Master.DOB ASC" | while read -r TransactionDate;
do
	...
	YLuna=$(mysql  --login-path=local -DSRG_Dev -N -e "SELECT YLuna from Lunas WHERE DOB = '$TransactionDate'")
			...
			mysql  --login-path=local -DSRG_Dev -N -e "UPDATE Master SET FY = '$FY',YLuna = '$YLuna' WHERE Master.DOB = '$TransactionDate'"
