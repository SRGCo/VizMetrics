...
mysql  --login-path=local --silent -DSRG_Dev -N -e "ALTER TABLE Master_temp ADD Account_status VARCHAR(15)"
trap 'failfunction ${?} ${LINENO} "$BASH_COMMAND"' ERR
...
# Create ACCOUNT STATUS
mysql  --login-path=local --silent -DSRG_Dev -N -e "ALTER TABLE Master_temp ADD Card_status VARCHAR(15)"
trap 'failfunction ${?} ${LINENO} "$BASH_COMMAND"' ERR


# AVOID DUPES DELETE SAME INTERVAL BACK
mysql  --login-path=local --silent -DSRG_Dev -N -e "DELETE FROM Master "
trap 'failfunction ${?} ${LINENO} "$BASH_COMMAND"' ERR
...
mysql  --login-path=local -DSRG_Dev -N -e "SELECT Master.DOB FROM Master WHERE Master.DOB IS NOT NULL
...
mysql  --login-path=local -DSRG_Dev -N -e "SELECT DISTINCT(CardNumber) FROM Master WHERE CardNumber IS NOT NULL
